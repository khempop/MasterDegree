...
public class oxgame {
    ...
    public static LinkedList<Integer> gameBoard = new LinkedList<Integer>();
    public static LinkedList<String> player = new LinkedList<String>();
    ...
    public static void recordPlayer(int score)throws IOException{
        ...
        if(score>gameBoard.get(gameBoard.size()-1)){
            name = getPlayer();
           
            tmpScore = gameBoard.get(gameBoard.size()-1);
            gameBoard.set(gameBoard.size()-1,score);
           
            tmpName = player.get(player.size()-1);
            player.set(player.size()-1,name);
        }
        for(int x=gameBoard.size()-1;x>=0;x--){
            if(x >= 1){
                if(gameBoard.get(x) > gameBoard.get(x-1)){
                    tmpScore = gameBoard.get(x-1);
                    gameBoard.set(x-1,gameBoard.get(x));
                    gameBoard.set(x, tmpScore);
                   
                    tmpName = player.get(x-1);
                    player.set(x-1,player.get(x));
                    player.set(x, tmpName);
                }
            }
        }
        System.out.println("High score");
        for(int y = 0;y<gameBoard.size();y++){
            System.out.print("name:"+player.get(y)+" score:");
            System.out.println(Integer.toString(gameBoard.get(y)));
        }
    }
